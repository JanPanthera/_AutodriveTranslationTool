# ctk_custom_textbox_builder.py ~ AutoDriveTranslationTool/src/widget_builder/ctk_custom_textbox_builder.py
...
class CustomTextboxBuilder(AbstractBuilder):
    def __init__(self, config_manager=None, localization_manager=None, logger=None):
        super().__init__(logger)
        self.logger = logger if logger else setup_default_logger('CustomTextboxBuilder')
    ...
    def create_widget(self, master, widget_properties):
        try:
            ...
            self.logger.error(f"Failed to create CustomTextbox: {e}")
            raise
