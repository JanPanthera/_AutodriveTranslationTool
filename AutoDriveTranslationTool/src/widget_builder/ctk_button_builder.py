# ctk_button_builder.py ~ AutoDriveTranslationTool/src/widget_builder/ctk_button_builder.py
...
class CtkButtonBuilder(AbstractBuilder):
    def __init__(self, config_manager=None, localization_manager=None, logger=None):
        super().__init__(logger)
        self.logger = logger if logger else setup_default_logger('CtkButtonBuilder')
    ...
    def create_widget(self, master, widget_properties):
        try:
            ...
            self.logger.error(f"Failed to create CTkButton: {e}")
            raise
